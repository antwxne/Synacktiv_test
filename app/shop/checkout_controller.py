...
def checkout_controller(basket_id: int, session: Session) -> List[Product]:
    try:
        basket_content: List[BasketRow] = session.exec(
            select(BasketRow).where(BasketRow.basket_id == basket_id)
        )

        product_list: List[Product] = []
        for basket_row in basket_content:
            product_to_update = session.exec(
                select(Product).where(Product.id == basket_row.product_id)
            ).one()
            product_to_update.amount -= basket_row.product_amount
            product_list.append(
                Product(id=basket_row.product_id, amount=basket_row.product_amount)
            )
